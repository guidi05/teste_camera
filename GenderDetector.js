...
export default function App() {
  ...
  return new faceapi.FaceDetector(faceDetectionNet, faceDetectionOptions);
};
...
if(status === 'granted' ){           //Permissao concedida
 ...
 setStartCamera(true);               //Startar camera com permissão concedida               
}
}
...
let takePic = async () => {               //Funcao tirar foto
  ...
  let newPhoto = await cameraRef.current.takePictureAsync(options);   //Acao de tirar foto 
...
else{                                           //Rosto detectado
      return (
<View 
style={{position: 'absolute',bottom: 0,flexDirection: "row",justifyContent:"center",marginBottom:" 5%",width: "60%"}}>
